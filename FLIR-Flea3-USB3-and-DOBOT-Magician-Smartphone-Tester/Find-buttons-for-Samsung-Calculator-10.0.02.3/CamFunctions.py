...
# Segmente l'image de la calculatrice
def segmentation(im):
    img = im[:,:,0]
    imb = im[:,:,1]
    imr = im[:,:,2]
    ...
    th = cv2.erode(th,cross,iterations = 1)
...
def getOnlyTouches(stats,centroidsTouches):
    areas = stats[:,4]
    med = np.median(areas)
    valid = np.array([area > 0.2 * med and area < 1.25 * med for area in areas])
...
def run_single_camera(cam):
    ...
    try:
        ...
        # Acquire images
        r1,centroidsTouches,screen_size = acquire_images(cam, nodemap, nodemap_tldevice)
        result &= r1
...
def run():
    ...
    cam = cam_list[0]
    print('Running...')
    r1,centroidsTouches,screen_size = run_single_camera(cam)
    result &= r1
    print('Completed... \n')
