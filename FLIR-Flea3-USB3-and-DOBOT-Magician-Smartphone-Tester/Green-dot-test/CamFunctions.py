...
# Détermine les coordonnées du point vert (cible)
def FindGreenTarget(dst):
    hsv = cv2.cvtColor(dst,cv2.COLOR_BGR2HSV)
    lower_green = np.array([50,35,35])
    upper_green = np.array([120,255,255])
    mask_g = cv2.inRange(hsv,lower_green,upper_green)
    res = cv2.bitwise_and(dst,dst,mask = mask_g)
    ret,th = cv2.threshold(res[:,:,0],5,255,cv2.THRESH_BINARY)
    ...
    cx = M["m10"]/M["m00"]
    cy = M["m01"]/M["m00"]
    print("Target: {},{}".format(cx,cy))
    return [cx,cy]

def dist(pt1,pt2):
    return math.sqrt((pt1[0] - pt2[0]) ** 2 + (pt1[1] - pt2[1]) ** 2)

def GreenCoord(target,centroids):
    ...
    print("Coordinates vert : {}".format([c1,c2]))
    print("Dimensions écran : {}".format([dist(centroids[0],centroids[2]),dist(centroids[0],centroids[1])]))
    return [c1,c2]
...
def acquire_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        print("Please ensure you have caibration screen image downloaded. Then put it in full screen to avoid mistakes.")
        print("Press c to calibrate and then q when you are done and want to use Dobot.")
        while(1):
            try:
    ...
    return result,green,screen_size
...
def run_single_camera(cam):
    ...
    try:
        ...
        # Acquire images
        r1,green,screen_size = acquire_images(cam, nodemap, nodemap_tldevice)
        result &= r1
...
def run():
    ...
    cam = cam_list[0]
    print('Running...')
    r1,green,screen_size = run_single_camera(cam)
    result &= r1
    print('Completed... \n')
    ...
    input('Done! Press Enter to exit camera calibration and start usind Dobby...')
    return result,green,screen_size
